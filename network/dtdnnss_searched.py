...
from nas_core.layer.common_layer.dtdnnss_v1 import DTDNNSS_V1_

from nas_core.layer.common_layer.tdnn import TDNN_, StatisticsPooling
...
class DtdnnssBase_v1(nn.Module):
    def __init__(self, num_class, in_channels=30, mid_channels=None, reduction_channels=None, chromosome=None,
                 feature_dim=512, loss_type='aam_loss'):
        super(DtdnnssBase_v1, self).__init__()
        ...
        # TODO Layer 2
        self.layer2 = DTDNNSS_V1_(in_channels=128, mid_channels=mid_channels[chromosome[0]],
                                  reduction_channels=reduction_channels[chromosome[0]],
                                  out_channels=64, kernel_size=3,
                                  dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 3
        self.layer3 = DTDNNSS_V1_(in_channels=192, mid_channels=mid_channels[chromosome[1]],
                                  reduction_channels=reduction_channels[chromosome[1]],
                                  out_channels=64, kernel_size=3,
                                  dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 4
        self.layer4 = DTDNNSS_V1_(in_channels=256, mid_channels=mid_channels[chromosome[2]],
                                  reduction_channels=reduction_channels[chromosome[2]],
                                  out_channels=64, kernel_size=3,
                                  dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 5
        self.layer5 = DTDNNSS_V1_(in_channels=320, mid_channels=mid_channels[chromosome[3]],
                                  reduction_channels=reduction_channels[chromosome[3]],
                                  out_channels=64, kernel_size=3,
                                  dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 6
        self.layer6 = DTDNNSS_V1_(in_channels=384, mid_channels=mid_channels[chromosome[4]],
                                  reduction_channels=reduction_channels[chromosome[4]],
                                  out_channels=64, kernel_size=3,
                                  dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 7
        self.layer7 = DTDNNSS_V1_(in_channels=448, mid_channels=mid_channels[chromosome[5]],
        ...
        # TODO Layer 9
        self.layer9 = DTDNNSS_V1_(in_channels=256, mid_channels=mid_channels[chromosome[6]],
                                  reduction_channels=reduction_channels[chromosome[6]],
                                  out_channels=64, kernel_size=3,
                                  dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 10
        self.layer10 = DTDNNSS_V1_(in_channels=320, mid_channels=mid_channels[chromosome[7]],
                                   reduction_channels=reduction_channels[chromosome[7]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 11
        self.layer11 = DTDNNSS_V1_(in_channels=384, mid_channels=mid_channels[chromosome[8]],
                                   reduction_channels=reduction_channels[chromosome[8]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 12
        self.layer12 = DTDNNSS_V1_(in_channels=448, mid_channels=mid_channels[chromosome[9]],
                                   reduction_channels=reduction_channels[chromosome[9]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 13
        self.layer13 = DTDNNSS_V1_(in_channels=512, mid_channels=mid_channels[chromosome[10]],
                                   reduction_channels=reduction_channels[chromosome[10]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 14
        self.layer14 = DTDNNSS_V1_(in_channels=576, mid_channels=mid_channels[chromosome[11]],
                                   reduction_channels=reduction_channels[chromosome[11]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 15
        self.layer15 = DTDNNSS_V1_(in_channels=640, mid_channels=mid_channels[chromosome[12]],
                                   reduction_channels=reduction_channels[chromosome[12]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 16
        self.layer16 = DTDNNSS_V1_(in_channels=704, mid_channels=mid_channels[chromosome[13]],
                                   reduction_channels=reduction_channels[chromosome[13]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 17
        self.layer17 = DTDNNSS_V1_(in_channels=768, mid_channels=mid_channels[chromosome[14]],
                                   reduction_channels=reduction_channels[chromosome[14]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 18
        self.layer18 = DTDNNSS_V1_(in_channels=832, mid_channels=mid_channels[chromosome[15]],
                                   reduction_channels=reduction_channels[chromosome[15]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 19
        self.layer19 = DTDNNSS_V1_(in_channels=896, mid_channels=mid_channels[chromosome[16]],
                                   reduction_channels=reduction_channels[chromosome[16]],
                                   out_channels=64, kernel_size=3,
                                   dilation=(1, 3), bias=False, in_shape=LEN_FRAME, out_shape=LEN_FRAME)

        # TODO Layer 20
        self.layer20 = DTDNNSS_V1_(in_channels=960, mid_channels=mid_channels[chromosome[17]],
...
if __name__ == "__main__":
    ...
    inputs = torch.ones([2, 30, 200])

    # # dtdnnss_2.6M_eer1.251
    # model = DtdnnssBase_v1(num_class=7323,
    #                        mid_channels=[64, 64, 96, 96, 128, 128],
    #                        reduction_channels=[32, 64, 32, 64, 32, 64],
    #                        chromosome=[0, 4, 1, 2, 2, 0, 2, 2, 3, 2, 2, 2, 1, 3, 2, 5, 1, 4],
    #                        feature_dim=128)
    # ckpt_path = '../checkpoint/dtdnnss_2.6M_eer1.251.pth'

    # # dtdnnss_2.9M_eer1.225
    # model = DtdnnssBase_v1(num_class=7323,
    #                        mid_channels=[64, 64, 96, 96, 128, 128],
    #                        reduction_channels=[32, 64, 32, 64, 32, 64],
    #                        chromosome=[4, 3, 5, 4, 5, 3, 1, 3, 0, 3, 4, 2, 5, 2, 2, 2, 5, 5],
    #                        feature_dim=128)
    # ckpt_path = '../checkpoint/dtdnnss_2.9M_eer1.225.pth'

    # dtdnnss_3.1M_eer1.14
    model = DtdnnssBase_v1(num_class=7323,
                           mid_channels=[64, 96, 128, 128, 160],
                           reduction_channels=[32, 64, 32, 64, 32],
                           chromosome=[3, 4, 2, 2, 2, 1, 1, 3, 1, 0, 0, 3, 3, 2, 4, 4, 3, 3],
                           feature_dim=128)
    ckpt_path = '../checkpoint/dtdnnss_3.1M_eer1.14.pth'

    model.load_state_dict(torch.load(ckpt_path, map_location='cpu')[CheckpointKey.ModelState], strict=False)
